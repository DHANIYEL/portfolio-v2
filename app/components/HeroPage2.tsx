...
const Hero = () => {
  ...
  const handleCardClick = () => {
    ...
    if (cardRef.current) {
      ...
      const scaleX = viewportWidth / cardWidth;
      const scaleY = viewportHeight / cardHeight;
      const finalScale = Math.max(scaleX, scaleY) * 1.1; // 1.1 for slight overflow

      // Calculate center position
      const centerX = viewportWidth / 2;
      const centerY = viewportHeight / 2;

      // Calculate translation to center (accounting for card's origin point)
